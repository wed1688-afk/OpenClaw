...
def _script():
    ...
    steps = [
        ...
        (298, "UserPromptSubmit", {"prompt": "Stream office state over SSE and keep the ledger append-only", "source": "direct"}),
    ]
    ...
    steps += tool(268, "Task", {"description": "map the ledger readers", "prompt": "find every reader of the ledger"})
    ...
    flat.append((238, dict(sub, hook_event_name="SubagentStop", last_assistant_message="Three readers, all in office_server.py")))
    ...
    tail += tool(196, "Bash", {"command": "python3 -m unittest -q", "description": "run the tests"}, ok=False, seconds_open=12)
    tail += tool(170, "Edit", {"file_path": "scripts/office_state.py"})
    tail += tool(150, "Bash", {"command": "python3 -m unittest -q", "description": "run the tests"}, seconds_open=11)
    ...
    flat.append((112, dict(base, hook_event_name="UserPromptSubmit", prompt="Looks good -- now draw the floor plan", source="direct")))
    ...
    flat.append((8, dict(base, hook_event_name="PreToolUse", tool_name="Bash",
                         tool_input={"command": "python3 scripts/office_server.py state --brief", "description": "check the floor"})))
    return flat
